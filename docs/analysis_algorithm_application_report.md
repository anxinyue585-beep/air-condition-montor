...
监督学习任务采用“当前月特征预测下月结果”的方式构造样本，共 660 条，其中训练集 540 条，测试集 120 条。
...
测试集 120 条，其中正样本 17 条。不同参数下结果如下：

| lambda | Accuracy | Precision | Recall | F1 |
|---:|---:|---:|---:|---:|
| 0.000 | 0.9583 | 1.0000 | 0.7059 | 0.8276 |
| 0.001 | 0.9583 | 1.0000 | 0.7059 | 0.8276 |
| 0.010 | 0.9583 | 1.0000 | 0.7059 | 0.8276 |
| 0.100 | 0.9583 | 1.0000 | 0.7059 | 0.8276 |
| 1.000 | 0.9583 | 1.0000 | 0.7059 | 0.8276 |

最佳模型 F1 为 0.8276，准确率为 0.9583，说明模型能够较好识别下月污染风险；Precision 为 1.0000，表示被预测为污染风险的样本没有误报，但 Recall 为 0.7059，说明仍有部分污染月份未被提前捕捉。
...
| alpha | MAE | RMSE | R2 |
|---:|---:|---:|---:|
| 0.0 | 0.9849 | 1.1988 | 0.9971 |
| 0.1 | 0.9579 | 1.1784 | 0.9972 |
| 1.0 | 0.9142 | 1.1277 | 0.9974 |
| 10.0 | 0.8495 | 1.0608 | 0.9977 |
| 100.0 | 1.9054 | 2.4033 | 0.9882 |

最佳参数为 `alpha = 10.0`，测试集 MAE 为 0.8495，RMSE 为 1.0608，R2 为 0.9977。相比“直接用当前月 AQI 预测下月 AQI”的基线模型，RMSE 从 7.3321 降低到 1.0608。

结论：污染物均值、优良率、月份周期和区域特征对下月 AQI 具有较强预测能力。`alpha = 100` 时误差明显升高，说明过强正则化会造成欠拟合。
...
本项目已完成数据分析算法应用指标：基础层实现 Top-N 风险排名，数据挖掘层实现 K-Means 城市污染画像聚类，机器学习层实现 Logistic Regression 下月污染风险分类，并额外实现 Ridge Regression 下月 AQI 时间序列预测。模型包含参数对比、训练测试划分、误差指标和结果解释，满足评分细则中对算法原理、实现过程、参数设置和实验结果分析的要求。
